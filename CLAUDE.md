...
1. 知り合い店舗1-2件に無料導入（実績作り）
2. 投稿→GBP/SNS自動展開の実装確認（API連携）
3. 夜間レポート生成機能（cron-job.org）
...
### 📝 議事録自動生成（2026/01/20追加）

**需要スコア**: 5/5（DEMAND_ANALYSIS_2026-01-20.mdより）

**実装済み**:
- `/minutes` ページ: 文字起こし入力 + 会議タイプ選択 + 構造化結果表示
- `/api/minutes` API: Claude API (claude-3-5-haiku-latest)で議事録抽出
- 5種類の会議タイプ対応（一般/朝会/振り返り/企画/1on1）
- 構造化出力: 概要、決定事項、アクションアイテム、次回会議
- セクション別コピーボタン + 全文コピー機能
